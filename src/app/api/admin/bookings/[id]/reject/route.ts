...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const txn = db.transaction(() => {
      ...
      db.prepare(`
        UPDATE bookings SET status = 'rejected', cancellation_reason = ?, updated_at = ? WHERE id = ?
      `).run(reason || 'Rejected by admin', now, id);

      createNotification(
        booking.requester_id,
        'booking_status',
        id,
        `Your booking has been rejected. Reason: ${reason || 'Not specified'}`
      );

      return db.prepare('SELECT * FROM bookings WHERE id = ?').get(id);
    });

    const rejectedBooking = txn();
